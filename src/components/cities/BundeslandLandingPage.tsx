...
import { areCityPagesPubliclyAccessible, getMapMarkers, hrefForCity } from "@/lib/cities/citiesMeta";
import type { BundeslandPageRecord } from "@/lib/cities/bundeslandPages";
import type { CityFacts } from "@/lib/cities/types";
import { absoluteUrl } from "@/lib/site";
import { useLang } from "@/lib/LanguageProvider";

type BundeslandLandingPageProps = {
  page: BundeslandPageRecord;
  cities: CityFacts[];
};

export function BundeslandLandingPage({
  page,
  cities,
}: BundeslandLandingPageProps) {
  const { t } = useLang();
  const bp = t.cities.bundeslandPage;
  const cityPagesPublic = areCityPagesPubliclyAccessible();
  const { content } = page;
  const markers = getMapMarkers().map((m) => ({
    slug: m.slug,
    name: m.name,
    x: m.x,
    y: m.y,
  ...
  const jsonLd = JSON.stringify([
    {
      ...
      provider: { "@type": "Organization", name: "Verdex" },
      areaServed: {
        "@type": "State",
        name: page.name,
      },
    ...
    },
    {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 3,
          name: page.name,
          item: absoluteUrl(`/deutschlandweit/bundesland/${page.slug}`),
        },
      ],
    },
  ]);
    ...
    <article className="relative pt-40 pb-24 lg:pt-48 lg:pb-32">
      <div className="mx-auto max-w-5xl px-6 lg:px-10">
        <BackLink
          ...
          label={bp.backToDeutschlandweit}
        />
        ...
        <div className="mt-12 grid grid-cols-1 items-start gap-12 lg:mt-16 lg:grid-cols-12 lg:gap-16">
          <FadeIn className="lg:col-span-7">
            <p className="text-xs font-medium uppercase tracking-[0.22em] text-neutral-500 dark:text-neutral-400">
              {bp.eyebrow.replace("{bundesland}", page.name)}
            </p>
            <h1 className="mt-6 max-w-3xl font-serif text-4xl leading-[1.05] tracking-tight text-neutral-900 dark:text-neutral-100 sm:text-5xl lg:text-6xl">
              {bp.heroTitle.replace("{bundesland}", page.name)}
            </h1>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3 text-sm text-neutral-500 dark:text-neutral-400">
              <span className="rounded-full border border-neutral-200 px-3 py-1 dark:border-white/10">
                {page.populationLabel}
              </span>
              <span className="rounded-full border border-neutral-200 px-3 py-1 dark:border-white/10">
                {page.stadtTyp}
              </span>
              {page.primaryLegalAreas.map((area) => (
                <span
                  key={area}
                  className="rounded-full border border-neutral-200 px-3 py-1 dark:border-white/10"
                >
                  {area}
                </span>
              ))}
          ...
          <FadeIn delay={0.1} className="lg:col-span-5">
            <GermanyCityMap
              markers={markers}
              highlightSlugs={page.mapHighlightSlugs}
              ariaLabel={bp.mapAria.replace("{bundesland}", page.name)}
            />
          </FadeIn>
        </div>
        ...
        {cities.length > 0 && (
          <Section
            eyebrow={t.cities.regionsEyebrow}
            title={bp.citiesTitle.replace("{bundesland}", page.name)}
          >
            ...
            <ul className="mt-8 grid gap-4 sm:grid-cols-2">
              {cities.map((city) => {
                const cardClassName =
                  "flex flex-col rounded-xl border border-neutral-200 p-5 dark:border-white/10" +
                  (cityPagesPublic
                ...
                return (
                  <li key={city.slug}>
                    {cityPagesPublic ? (
                      <Link href={hrefForCity(city.slug)} className={cardClassName}>
                        {cardContent}
                      </Link>
                    ) : (
        ...
        <Section
          ...
          title={bp.courtsTitle.replace("{bundesland}", page.name)}
        >
          <p className="max-w-3xl text-lg leading-relaxed text-neutral-600 dark:text-neutral-300">
            {content.courtsNarrative}
          </p>
          <dl className="mt-8 grid gap-4 sm:grid-cols-3">
            <FactCard label="Amtsgericht" value={page.courts.amtsgericht} />
            <FactCard label="Landgericht" value={page.courts.landgericht} />
            <FactCard
              label="Oberlandesgericht"
              value={page.courts.oberlandesgericht}
            />
          </dl>
          {page.courts.besonderheiten && page.courts.besonderheiten.length > 0 && (
            <ul className="mt-6 space-y-2 text-sm text-neutral-500 dark:text-neutral-400">
              {page.courts.besonderheiten.map((note) => (
        ...
        <Section
          ...
          title={bp.economyTitle.replace("{bundesland}", page.name)}
        >
          <p className="max-w-3xl text-lg leading-relaxed text-neutral-600 dark:text-neutral-300">
            {content.economyNarrative}
          </p>
          <div className="mt-8 flex flex-wrap gap-2">
            {page.economy.sectors.map((sector) => (
              <span
                key={sector}
        ...
        <Section
          ...
          title={bp.servicesTitle.replace("{bundesland}", page.name)}
        >
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            ...
            <MiniFeature
              icon={ShieldCheck}
              label={t.solutions.authority.shieldFeatures[0].title}
            />
        ...
        <CityFaq
          items={content.faq}
          cityName={page.name}
          titleOverride={bp.faqTitle.replace("{bundesland}", page.name)}
        />

        <Section
          eyebrow={t.cities.sections.whyVerdex}
          title={bp.whyVerdexTitle.replace("{bundesland}", page.name)}
        >
          <p className="max-w-3xl text-lg leading-relaxed text-neutral-600 dark:text-neutral-300">
        ...
        <div className="mt-16 flex flex-wrap items-center gap-x-8 gap-y-5 border-t border-neutral-100 pt-10 dark:border-white/10 lg:mt-20">
          ...
          >
            {bp.cta.replace("{bundesland}", page.name)}
            <ArrowRight
              size={16}
